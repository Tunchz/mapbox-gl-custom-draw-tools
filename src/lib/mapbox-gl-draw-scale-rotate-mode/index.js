...
SRMode._createRotationPoint = function (
  ...
  var cR1 = destination(rotCenter, distance1, heading, unit).geometry.coordinates;
...
SRMode.computeAxes = function (state, polygon) {
  ...
  for (var i = 0; i < n; i++) {
    ...
    scaleCenters[i] = c0;
    distances[i] = distance(point(c0), point(c1), unit);
  }
...
SRMode.dragScalePoint = function (state, e, delta) {
  ...
  var dist = distance(center, m1, unit);
