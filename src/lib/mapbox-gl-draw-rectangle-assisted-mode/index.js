  ...
  onMouseMove: function (state, e) {
    ...
    if (state.currentVertexPosition && state.currentVertexPosition > 0) {
      ...
      this.popupWindow
        ...
        .setHTML(`<div class="rec-tooltip" style="margin-top:-8px;margin-bottom:-17px;color:#000000;">${state.angle?('มุม ： ' + state.angle + ''):''} ดีกรี</div>`)
        .addTo(this.map);
    }
    ...
    if (state.currentVertexPosition === 2) {
      ...
      this.popupWindow
        .setLngLat([e.lngLat.lng, e.lngLat.lat])
        .setHTML(`<div class="rec-tooltip" style="margin-top:-8px;margin-bottom:-13px;color:#000000;">${state.angle?('มุม  ：' + state.angle + ' ดีกรี<br>'):''}พื้นที่：${area.toFixed(2)} m2</div>`)
        .addTo(this.map);
      if (getpXY3) {
        state.rectangle.updateCoordinate("0." + (state.currentVertexPosition + 1), getpXY3[0], getpXY3[1]);
      }
